  ...
  return internalHeaders.value.filter(
    (header) =>
      ...
      selectedHeaders.value.some(
        (selectedHeader) => selectedHeader.value === header.value
      )
  );
});
...
function exportNames() {
  return {
    xlsxFileName: `export_${props.uniqueName}`,
    xlsxSheetName: `${props.uniqueName.substring(0, 14)}`,
  };
}
const { xlsxFileName, xlsxSheetName } = exportNames();
          ...
          <th v-else class="pa-2 text-left">
            ...
            >
              <VBtn
                color="default"
                variant="text"
                :icon="options.exportToExcelIcon"
              />
            </ExportToExcel>
            <VBtn
              v-if="hasFilterable"
